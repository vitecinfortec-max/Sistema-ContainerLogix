#!/usr/bin/env python3
"""
Importa a lista de transportadoras do sistema anterior a partir do XLSX
exportado, deduplicando por nome (nao ha CNPJ para todas as linhas).
"""
import asyncio
import os
import sys
...
EXCEL_PATH = r"C:\Users\victo\OneDrive\Área de Trabalho\Lista de cadrasto de transportadora.xlsx"
...
async def import_transport_companies(dry_run: bool = False):
    ...
    for _, row in df.iterrows():
        ...
        try:
            existing = await db.transport_companies.find_one({"name": name}, {"_id": 0, "id": 1})
            if existing:
...
if __name__ == "__main__":
    ...
    asyncio.run(import_transport_companies(dry_run=dry_run))
