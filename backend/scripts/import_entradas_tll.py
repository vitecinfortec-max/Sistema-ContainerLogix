#!/usr/bin/env python3
"""
Importa o histórico de movimentações (ENTRADA + SAÍDA) do relatório
"Registro de entrada - TLL Container.xlsx" - preserva o transaction_id
original de cada linha, EXCETO quando ele já está em uso por um evento
diferente no banco (colisão de numeração entre relatórios de clientes
distintos) - nesse caso atribui um novo número sequencial, continuando
do maior transaction_id já conhecido (existente no banco ou no próprio
arquivo), pra nunca perder um registro nem sobrescrever outro.
"""
import asyncio
import os
import sys
...
EXCEL_PATH = r"C:\Users\victo\OneDrive\Área de Trabalho\Registro de entrada - TLL Container.xlsx"
...
# alguns números já vêm com o traço/dígito no arquivo, outros não - normaliza todos
# pro mesmo formato usado no resto do sistema.
_LETTER_VALUES = {
...
def format_container_number(raw: str) -> str:
    ...
    if len(s) == 11 and s[:4].isalpha() and s[4:].isdigit():
        ...
        return raw  # formato inesperado (ex: container avulso tipo "BAÚ MARROM") - mantém como veio
...
async def import_movements(dry_run: bool = False):
    ...
    file_ids = set(int(v) for v in df["ID Trans."].tolist())

    # Próximo número livre pra colisões: acima de tudo que já existe no banco
    # E de tudo que este arquivo vai preservar (pra nunca colidir com uma
    # linha original deste mesmo arquivo que ainda vai ser inserida).
    next_fresh = max(existing_ids | file_ids, default=0) + 1

    imported = 0
    skipped = 0
    errors = 0
    reassigned = []
    max_transaction_id_used = 0

    for row_idx, row in df.iterrows():
        original_id = int(row["ID Trans."])
        try:
            existing = await db.movements.find_one({"transaction_id": original_id}, {"_id": 0, "id": 1})
            if existing:
                transaction_id = next_fresh
                next_fresh += 1
                reassigned.append((original_id, transaction_id))
            else:
            ...
            doc = {
                ...
                "currency": "BRL",
                "observations": (
                    f"Importado do sistema anterior (Registro de entrada - TLL Container.xlsx)"
                    + (f" - ID Trans. original era #{original_id}, renumerado por colisão" if transaction_id != original_id else "")
                ),
            ...
            imported += 1
            max_transaction_id_used = max(max_transaction_id_used, transaction_id)

            if imported % 100 == 0:
                print(f"Progresso: {imported} movimentações importadas...")

        except Exception as e:
            print(f"Linha {row_idx + 2}: erro no transaction_id {original_id} - {e}")
            errors += 1

    if not dry_run and max_transaction_id_used > 0:
        counter = await db.counters.find_one({"_id": "transaction_id"})
        current_seq = counter["seq"] if counter else 0
        if max_transaction_id_used > current_seq:
            await db.counters.update_one(
                {"_id": "transaction_id"},
                {"$set": {"seq": max_transaction_id_used}},
                upsert=True,
            )
            print(f"Contador transaction_id atualizado de {current_seq} para {max_transaction_id_used}")
        else:
            print(f"Contador transaction_id mantido em {current_seq} (>= {max_transaction_id_used})")
    ...
    print(f"  - Renumeradas por colisão: {len(reassigned)}")
    for orig, new in reassigned:
        print(f"      #{orig} -> #{new}")
    print("=" * 50)
