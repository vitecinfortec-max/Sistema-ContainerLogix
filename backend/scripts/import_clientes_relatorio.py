#!/usr/bin/env python3
"""
Importa a lista de clientes do sistema anterior a partir do XLSX exportado.
"""
import asyncio
import os
import sys
...
EXCEL_PATH = r"C:\Users\victo\OneDrive\Área de Trabalho\Lista de cadrasto de cliente.xlsx"
...
async def import_clients(dry_run: bool = False):
    ...
    for _, row in df.iterrows():
        ...
        try:
            existing = await db.clients.find_one({"name": name}, {"_id": 0, "id": 1})
            if existing:
...
if __name__ == "__main__":
    ...
    asyncio.run(import_clients(dry_run=dry_run))
