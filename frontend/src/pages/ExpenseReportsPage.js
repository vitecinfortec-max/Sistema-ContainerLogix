...
// Componente Autocomplete (mesmo padrão usado em Solicitação de Diária)
...
export default function ExpenseReportsPage() {
  ...
  const handleDelete = async (id) => {
    if (!confirm('Deseja realmente excluir esta prestação de contas?')) return;
  ...
  const handleComplete = async (id) => {
    if (!confirm('Concluir esta prestação de contas? A edição ficará bloqueada até reabrir.')) return;
    try {
